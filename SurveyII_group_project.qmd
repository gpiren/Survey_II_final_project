...
map_data <- world %>%
  left_join(legal_framework_data, by = c("name" = "country_name"))
...
ggplot(legal_framework_data, aes(x = region, y = LGBT..Policy.Index, fill = region)) +
  geom_boxplot() +
...
lm_model <- lm(LGBT..Policy.Index ~ qc19 + qc20 + qc15_1 + qc15_2 + qc15_3, data = legal_framework_data)
...
lm_model_interact <- lm(LGBT..Policy.Index ~ qc19 * region + qc20 * region, data = legal_framework_data)
