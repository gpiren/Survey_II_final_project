...
save(data_selected, file = "data_selected.rda")


```

Test for multicollinearity and linearity:

```{r}
library(car)

alias_info <- alias(lm(qc19_numeric ~ ., data = data_selected))
print(alias_info$Complete)  # Liste der abhängigen Variablen anzeigen

# Entferne diese abhängigen Variablen
dependent_vars <- colnames(alias_info$Complete)
data_selected <- data_selected[, !colnames(data_selected) %in% dependent_vars]


library(MASS)
best_model <- step(glm(qc19_numeric ~ ., data = data_selected, family = binomial), direction = "both")
...
-   No extremely high correlation (\>\|0.75\|) → No variables need to be removed immediately.
-   LGBT..Policy.Index has a moderate correlation with min_wage (0.43) & qc19_numeric (0.34) → This could be an important predictor
-   min_wage & AIC have a correlation of 0.57 → This is not critical, but should be tested in the model.
-   unemployment_rate is almost uncorrelated with all other variables → Could possibly be unimportant.
...
str(data_selected)
chisq.test(table(data_selected$qc19_numeric, data_selected$country_name))
chisq.test(table(data_selected$qc19_numeric, data_selected$region))


factor_vars <- names(data_selected)[sapply(data_selected, is.factor)]

# Leerer Vektor für p-Werte
chi_results <- numeric(length(factor_vars))
names(chi_results) <- factor_vars

# Berechne Chi-Quadrat-Test für jede Faktor-Variable
for (var in factor_vars) {
  test <- chisq.test(table(data_selected[[var]], data_selected$qc19_numeric))
  chi_results[var] <- test$p.value  # Speichere den p-Wert als numerischen Wert
}

# Sortiere die p-Werte in aufsteigender Reihenfolge
...
train_index <- createDataPartition(data_selected$qc19_numeric, p = 0.8, list = FALSE)
...
glmm_model <- glmer(qc19_numeric ~ LGBT..Policy.Index + min_wage + unemployment_rate + (1 | country_name),
                    data = data_selected, 
                    family = binomial, 
