...
# Crear variable de nombre de país
data <- data %>%
...
data_clean <- data %>%
  select(country_name,  # País del encuestado
         qc19,  # Apoyo a personas trans en documentos oficiales
         d11,   # Edad del encuestado
         d10,   # Género
         d8,    # Nivel educativo
         qc1_4, qc1_8, qc2_4, qc2_6, qc4_8,  # Discriminación y percepción de desigualdad
         qc15_1, qc15_2, qc15_3,  # Opiniones sobre derechos LGB
         qc17_3, qc17_4, qc18_2, qc18_3,  # Actitudes sobre diversidad
         qc20)  # Opinión sobre tercera opción de género en documentos oficiales

#--------------------------------------------------------
#verificacion
# Ver los valores únicos de cada variable antes de transformarlas en factores
unique(data_clean$qc19)  # Debería contener valores como 1, 2, 3, 4
unique(data_clean$qc1_4) # Aquí vemos si hay más o menos de 3 valores esperados
...
#-------------------------------------------------------------
# 📌 Transformar variables en factores con etiquetas descriptivas
...
data_clean <- data_clean %>%
  mutate(
    ...
    d10 = factor(d10, levels = c(1,2),
                 labels = c("Man", "Woman")),
    d8 = factor(d8),  # Mantener educación como factor
    
    # Variables con solo 2 niveles ("Sí", "No")
    ...
    # Variables con 3 niveles ("Sí", "No", "No sabe")
...
```{r}
# Número total de observaciones (encuestados)
n_respondents <- nrow(data_clean)

# Número de países en la encuesta
n_countries <- length(unique(data_clean$country_name))

# Mostrar resultados
cat("Número total de encuestados:", n_respondents, "\n")
cat("Número total de países:", n_countries, "\n")
...
ggplot(data_clean, aes(x = qc19, fill = qc19)) +
  geom_bar() +
  labs(title = "Distribución del Apoyo a Personas Trans",
       x = "Nivel de Apoyo", y = "Número de Respuestas") +
  theme_minimal() +
...
ggplot(data_clean, aes(x = fct_reorder(country_name, as.numeric(qc19)), fill = qc19)) +
  ...
  labs(title = "Apoyo a Personas Trans por País",
       x = "País", y = "Proporción de Respuestas", fill = "Apoyo") +
  theme_minimal() +
  coord_flip()
...
The significant percentage of “Don't Know” (DK) responses in certain countries suggests a lack of awareness or uncertainty and lack of public discourse or education on transgender issues; which may indicate a need for greater public discussion and education regarding transgender rights.Bulgaria, Cyprus, and Poland might benefit from more awareness campaigns to address knowledge gaps and clarify public opinions on transgender rights.
...
ggplot(data_clean, aes(x = qc19, fill = d10)) +
  ...
  labs(title = "Apoyo a Personas Trans por Género",
       x = "Nivel de Apoyo", y = "Número de Respuestas", fill = "Género") +
  theme_minimal()
...
ggplot(data_clean, aes(x = qc1_4, fill = qc1_8)) +
  geom_bar(position = "fill") +
  labs(title = "Percepción de Discriminación: Orientación Sexual vs. Ser Trans",
       x = "Percepción de Discriminación por Orientación Sexual",
       y = "Proporción", fill = "Discriminación hacia Personas Trans") +
  theme_minimal()
...
# 📌 Normalize by total respondents per country (percentage)
...
# 📌 Visualization: Countries with the highest perceived discrimination
...
```{r}
# Crear una tabla resumen del apoyo a los derechos LGBTI por país
...
ggplot(support_lgbti_country, aes(x = reorder(country_name, percentage), y = percentage)) +
  ...
  coord_flip() +
  labs(title = "Apoyo a los Derechos LGBTI por País",
       x = "País",
...
```{r}
# Comparación entre Europa Occidental y Europa del Este
data_clean <- data_clean %>%
...
# Calcular el porcentaje de apoyo en cada región
...
# Visualización del apoyo a derechos LGBTI por región
ggplot(support_by_region, aes(x = region, y = percentage, fill = region)) +
  geom_bar(stat = "identity") +
  labs(title = "Apoyo a los Derechos LGBTI por Región",
       x = "Región",
       y = "Porcentaje de Apoyo") +
  theme_minimal()
...
# Convertir qc19 a numérico asignando valores
data_clean <- data_clean %>%
...
Since the ANOVA is significant, we must verify its assumptions before proceeding with further analysis. The steps you outlined are correct. We will perform the Levene's Test for Homogeneity of Variance and Tukey’s HSD Post-hoc Test for identifying which countries differ significantly. 
...
TukeyHSD(anova_model)  # Comparaciones múltiples entre países

# 3. Visualización con Boxplot
ggplot(data_clean, aes(x = country_name, y = qc19_num)) +
  geom_boxplot() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  labs(title = "Diferencias en Apoyo a los Derechos LGBTI entre Países",
       x = "País", y = "Nivel de Apoyo")
...
Since the Kruskal-Wallis test detected significant differences among countries, we need to perform Dunn’s post-hoc test to determine which specific countries differ from each other. This test will help us identify the exact pairs of countries with statistically significant differences in LGBTI support.
...
# Realizar la prueba post-hoc de Dunn con corrección de Bonferroni
...
ggplot(data_clean, aes(x = country_name, fill = as.factor(qc19))) +
  geom_bar(position = "fill") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  labs(title = "Distribución del Apoyo a los Derechos LGBTI por País",
       x = "País", y = "Proporción de Respuestas", fill = "Apoyo")
...
Finally, the model's residual deviance (50970.31) and AIC (51188.31) indicate how well the model fits the data. While the interaction terms add complexity and depth to the model, further refinement—such as assessing multicollinearity, checking for influential observations, or using a mixed-effects model—might be necessary to improve the model’s predictive accuracy and interpretability.
